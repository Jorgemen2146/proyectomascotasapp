...
public sealed class PetPhotosController : ControllerBase
{
        ...
        CancellationToken cancellationToken)
    {
        var command = new AddPetPhotoCommand(
            petId, request.FileName, request.ContentType, request.ImageBase64);
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (result.IsFailure)
        {
            return result.Error.Code switch
            {
                "Pet.NotFound" => NotFound(),
                "Pet.Unauthorized" => Forbid(),
                _ when result.Error.Type == DogPlatform.SharedKernel.Primitives.ErrorType.Failure
    ...
    /// <summary>Returns registered image content to its authenticated owner.</summary>
        ...
        CancellationToken cancellationToken)
    {
        var result = await _mediator.Send(
            new GetPetPhotoContentByIdQuery(petId, photoId), cancellationToken);

        if (result.IsFailure)
            return result.Error.Code == "Pet.Unauthorized" ? Forbid() : NotFound();
